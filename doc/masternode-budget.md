...
LasVegasCoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<lasvegascoin_address\> \<monthly_payment_lasvegascoin\> [use_ix(true|false)]
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<lasvegascoin_address\> \<monthly_payment_lasvegascoin\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` Las Vegas Coin.
 ...
 * vote-many          - Vote on a LasVegasCoin initiative
 * vote-alias         - Vote on a LasVegasCoin initiative
 * vote               - Vote on a LasVegasCoin initiative/budget
