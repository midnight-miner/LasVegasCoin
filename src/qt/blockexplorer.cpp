...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::VGS, nValue);
...
std::string BlockToString(CBlockIndex* pBlock)
{
    ...
    if (pBlock->nHeight == 0)
        ...
        Generated = GetBlockValue(pBlock->nHeight - 1, pBlock->IsProofOfStake());
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            ...
            QMessageBox::warning(this, "Las Vegas Coin Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
