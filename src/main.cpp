...
#if defined(NDEBUG)
#error "Las Vegas Coin cannot be compiled without assertions."
#endif
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!IsInitialBlockDownload() && !IsBlockValueValid(block, GetBlockValue(pindex->pprev->nHeight, pindex->pprev->IsProofOfStake()))) {
        return state.DoS(100,
            error("ConnectBlock() : reward pays too much (actual=%d vs limit=%d)",
                block.vtx[0].GetValueOut(), GetBlockValue(pindex->pprev->nHeight, pindex->pprev->IsProofOfStake())),
